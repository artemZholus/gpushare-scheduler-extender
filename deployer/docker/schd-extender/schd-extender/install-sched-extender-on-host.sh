...
sche-policy-config() {
    ...
    if [ ! -f $backup_dir/kube-scheduler.ori.yaml ];then
        ...
        log "Backup $backup_dir/kube-scheduler.ori.yaml"
    else
        cp /etc/kubernetes/manifests/kube-scheduler.yaml $backup_dir/kube-scheduler-$TIMESTAMP.yaml
        log "Backup $backup_dir/kube-scheduler-$TIMESTAMP.yaml"
    fi
    ...
    if [ ! -f $backup_dir/scheduler-policy-config.ori.json ];then
        if [ -f /etc/kubernetes/scheduler-policy-config.json ];then
            ...
            log "Backup $backup_dir/scheduler-policy-config.ori.json"
        fi
    else
        if [ -f /etc/kubernetes/scheduler-policy-config.json ];then
            cp /etc/kubernetes/scheduler-policy-config.json $backup_dir/scheduler-policy-config-$TIMESTAMP.json
            log "Backup $backup_dir/scheduler-policy-config-$TIMESTAMP.json"
        fi
    fi

    log "Configure shceduler extender"
    cp -f ./config/scheduler-policy-config.json /etc/kubernetes/scheduler-policy-config.json
    ...
    if ! grep 'policy-config-file=/etc/kubernetes/scheduler-policy-config.json' $dir/kube-scheduler.yaml; then
        ...
        log "Skip the kube-scheduler config, because it's already configured extender."
    fi
    ...
    if ! grep 'mountPath: /etc/kubernetes/scheduler-policy-config.json' $dir/kube-scheduler.yaml; then
        ...
        log "Skip the scheduler-policy-config mountPath, because it's already configured extender."
    fi
    ...
    if ! grep 'path: /etc/kubernetes/scheduler-policy-config.json' $dir/kube-scheduler.yaml; then
        ...
        log "Skip the scheduler-policy-config volumes, because it's already configured extender."
    fi
}
...
main() {
    ...
    if [ "$KUBE_VERSION" == "" ]; then
        # Using default cidr.
        log "KUBE_VERSION $KUBE_VERSION is not set."
        exit 1
    fi
